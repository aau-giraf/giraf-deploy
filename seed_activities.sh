...
add() {
  local offset=$1 start=$2 end=$3 pid=$4 title=$5
  local d=$(date -d "$MONDAY + $offset days" +%Y-%m-%d)
  local r=$(curl -sf -X POST "$WP_URL/weekplan/to-citizen/$CITIZEN_ID" \
    -H 'Content-Type: application/json' -H "$AUTH" \
    -d "{\"date\":\"$d\",\"startTime\":\"$start\",\"endTime\":\"$end\",\"pictogramId\":$pid,\"title\":\"$title\"}")
  local aid=$(echo "$r" | python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get('activityId', d.get('id','?')))")
  echo "  $d $start-$end $title -> #$aid"
  CREATED+=("$aid")
}

for day in 0 1 2 3 4; do
  echo "--- $(date -d "$MONDAY + $day days" +%A) ---"
  add $day "08:00" "08:30" "$P_BUS"   "Skolebus"
  add $day "09:00" "10:30" "$P_HAPPY" "Undervisning"
  add $day "11:30" "12:15" "$P_EAT"   "Frokost"
  add $day "13:00" "14:00" "$P_LUNCH" "Frikvarter"
done
